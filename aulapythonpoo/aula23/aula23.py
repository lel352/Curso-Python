...
@dataclass(eq=True, repr=True, order=True, frozen=False, init=True)  # Habilitando a ou não a criação de algumas métodos
class Pessoa:  # ja está criado: __init__(), __repr__()
    nome: str
    sobrenome: str = field(repr=False)

    def __post_init__(self):  # execultado logo depois do Init
